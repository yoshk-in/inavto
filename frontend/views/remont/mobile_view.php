<div class="content">
        <div class="dirs">
            ...
            <svg class="i arrow"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#keyboard-down"></use></svg><a href="<?= yii\helpers\Url::to(['/remont/index']); ?>">Ремонт Volvo</a>
            <svg class="i arrow"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#keyboard-down"></use></svg><a href="<?= yii\helpers\Url::to(['/remont/category', 'alias' => $model->alias]); ?>"><?=$model->title; ?></a>
        </div>
        <h1><?=$model->title; ?></h1>
        <div class="row">
	<div class="userCarSelect">
		...
		<select name="car" id="modelSelect">
                    ...
                    <option data-url="remont/<?=$value->alias; ?>" value="<?=$value->id; ?>" <?=$value->id == $model->id ? 'selected="selected"' : ''; ?>><?=$value->car->title; ?></option>
                        <?php endforeach; ?>
                    <?php endif; ?>
		</select>
		<select name="generation" id="generationSelect">
			<option value="" <?=!$f_gen ? 'selected="selected"' : ''; ?>>все поколения</option>
                ...
                <?php if($f_gen): ?>
                <a class="filterReset" id="filterReset" href="<?= yii\helpers\Url::to(['remont/category', 'alias' => $model->alias]); ?>">Сбросить уточнение</a>
                <?php endif; ?>
	</div>
</div>
<div class="row">
				...
				<div class="priceGroup table bordered">
                                        ...
                                        <?php if($j_v['info']): ?>
                                            <?php foreach($j_v['info'] as $e_k => $e_v): ?>
                                                ...
                                                ?>
                                                <div class="car"><?=$engines && !empty($engines) ? implode($engines, ', ') : 'Все поколения и моторы' ?> , </div>
                                                <?php
                                                        $price = array();
                                                        if(!empty($e_v['parts'])){
                                                    ...
                                                    ?>
                                                <div class="partsPrice">Зап.части: <span><?=$range; ?> <span class="ruble">p</span></span></div>
                                                <div class="workPrice">Работа: <span><?=$e_v['price']; ?> <span class="ruble">p</span></span></div>
                                            <?php endforeach; ?>
                                       <?php endif; ?>
					</div>
				</div>
