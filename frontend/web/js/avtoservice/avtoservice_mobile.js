...
avtoservice.prototype = {
    ...
    genSwitch: function() {
        ...
        // @test exchange #motorSwitch on #egineSet
        t.n.motorSwitch.html('');
        // var pattern =
        //  '<span class="selectBtn"> <input type="radio" name="engineSet" id="engine{id}" {checked} value="{id}"> <label for="engine{id}">{name}</label> </span>';
            ...
            if(genIndex in cars[carIndex].generations) {
                if('motors' in cars[carIndex].generations[genIndex]) {
                    for(m in cars[carIndex].generations[genIndex].motors) {
                        ...
                        t.n.motorSwitch.append(`<span class="selectBtn"> <input type="radio" name="engineSet" id="engine${motorData.id_motor}" value="${motorData.id_motor}"> <label for="engine${motorData.id_motor}">${motorData.motorName}${pwr}</label> </span>`);
        ...
        $(t.n.motorSwitch.find('input:first')).attr('checked', 'checked');
    ...
    calc: function() {
        ...
        // @test exchange #motorSwitch on #engineSet
        // t.d.motorId=parseInt(t.n.motorSwitch.val());
        t.d.motorId=parseInt(t.n.motorSwitch.find(':checked').val());
        console.log('selected motorSwitch id '+t.n.motorSwitch.find(':checked').val());
        ...
        t.d.motorName=t.n.motorSwitch.find(':checked + label').text();
        console.log('motorName: '+t.d.motorName);
        // switch image
    ...
    showResults: function(data) {
        ...
        t.countWorks = 0;
        var totalPrice=0;
        //@test disable recommended_works
        data.works.recommended = [];
        ...
        if(!data.works.mandatory.length && !data.works.recommended.length) {
            ...
            t.n.mandatory.table = $('<div class="listWorks table bordered">' +
                ...
                '<label for="mandatory_works">Стоимость</label></h3></div>' +
            ...
            // t.n.signatures = $('<div class="row header signatures"><div class="num">&nbsp;</div><div class="workCheck">&nbsp;</div><div class="workName">Наименование работ:</div><div class="workPrice">Стоимость работ:</div><div class="partsPrice">Стоимость запчастей:</div></div>');
...
console.log(wd)
        ...
        //@test exchange origin\analog parts order on one button
        $('#partsOrigin').click(function() {
            let parts = $(".listWorks .work .setsOfParts .select input[value^=original]");
            console.log(parts);
            parts.click();
        });
        $('#partsAnalog').click(function() {
            let parts = $(".listWorks .work .setsOfParts .select input[value^=analog]");
            console.log(parts);
            parts.click();
        });
    ...
    recalc: function() {
        ...
        $('.serviceResults .secondCol .totalPrice').html((sumMandatory+0).formatMoney(false));
    ...
    init: function() {
        ...
        t.n.motorSwitch = t.n.calc.find('#engineSet');
        // console.log('engine set id: ',t.n.motorSwitch);
        ...
        //mobile fix
        // $('#motorSwitch').siblings('span').children('[name="motor"]').change((e) => {
        //     console.log(e.target);
        //     $('#motorSwitch').val($(e.target).val());
        //     $('#motorSwitch').trigger('change');
        // });
        // bind events
        ...
        // init slider
        $('#distance').on('change', () => {
            t.d.range = parseInt($('#distance').val());
            t.calc();
        });
        // $('#slider').slider({
        //     animate: "fast",
